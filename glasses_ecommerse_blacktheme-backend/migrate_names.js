    ...
    try {
        ...
        for (const genderCat of genderCats) {
            ...
            for (const subsub of subSubCats) {
                ...
                const updateResult = await db.collection('products').updateMany({ category: subsub._id }, { $set: { category: targetSubId } });
            ...
            const looseProducts = await db.collection('products').updateMany({ category: genderCat._id }, { $set: { category: mainCategoryId } } // Or we could try to determine a better fallback, but the research said 0
            );
