...
async function seedCategories() {
    try {
        ...
        // 3. CONTACT LENSES
        const contactLenses = await Category.create({
            name: 'Contact Lenses',
            slug: 'contact-lenses',
            level: 'main',
        ...
        const mainCats = await Category.countDocuments({ level: 'main' });
        const subCats = await Category.countDocuments({ level: 'sub' });
        const subSubCats = await Category.countDocuments({ level: 'subsub' });
