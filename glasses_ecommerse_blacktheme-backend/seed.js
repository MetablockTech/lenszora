...
async function seed() {
    try {
        ...
        await Sale_1.Sale.deleteMany({});
        await Setting_1.Setting.deleteMany({});
        ...
        // Create Brands
        const createdBrands = await Brand_1.Brand.insertMany(brandsData);
        console.log(`✓ Created ${createdBrands.length} brands`);
        const getBrand = (name) => createdBrands.find(b => b.name === name)?._id;
        // Create Categories recursively
        const categoryMap = {};
        async function createCategories(list, parentId = null, level = 'main') {
            for (const item of list) {
                const cat = await Category_1.Category.create({
                    name: item.name,
                    slug: item.slug,
                    parentId,
                    level,
        ...
        const createdProducts = await Product_1.Product.find({});
